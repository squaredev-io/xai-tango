...
class ShapRequest(BaseModel):
    plot_type: Literal["summary", "bar", "waterfall", "heatmap", "beeswarm"] = Field(
        ..., description="Type of SHAP plot", example="summary"
    )
...
class ConvolutionalFeaturesRequest(BaseModel):
    ...
    isolation: bool = Field(
        ..., description="Flag indicating whether to isolate convolutional features (True) or not (False)."
    )
...
class IntegratedGradientsResponse(BaseModel):
    gradients: list[str] = Field(
        ..., description="List of base64-encoded images of the Integrated Gradients visualization."
    )
...
class ShapSingleSampleResponse(BaseModel):
    shap_plots: list[str] = Field(
        ..., description="List of base64-encoded SHAP visualizations for the selected sample."
    )
