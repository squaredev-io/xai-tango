...
from xaivision.utils import load_models,model_details,sample_details, conv2d_feature_vis_extra_layers,conv2d_feature_vis_no_extra_layers, find_components, MedPCacheDataset_normalised
    ...
    if functionality == "Model Details":
        ...
        if sample_number is not None:
            ...
            if len(grads) == 1:  
                ...
                num_rows = (len(grads) + 1) // 2  # Calculate the number of rows needed
                fig, axes = plt.subplots(num_rows, 2, figsize=(10, 5 * num_rows))
                fig.tight_layout(pad=3.0)
