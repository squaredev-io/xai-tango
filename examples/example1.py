...
from xaivision.utils import load_sample,check_model_data_compatibility,model_details,sample_details,check_onnx_torch_out , conv2d_feature_vis_extra_layers,conv2d_feature_vis_no_extra_layers, find_components
from xaivision.xai_tools import vision_shap, integrated_grad, deeplift, shap_overview
...
if __name__ == '__main__':
    ...
    optional.add_argument('-s', '--sample', type=int, default=4,
    ...
    if not check_model_data_compatibility(torch_model,model_input.shape, ground_truth.shape):
    ...
    arrays, names = conv2d_feature_vis_no_extra_layers(torch_model,model_input)
    ...
    plt.savefig(folder_path_sample+"/activation_map_with_no_extra_layers.png")
    ...
    for i in range(components):
        ...
        plt.savefig(folder_path_components+"/component_" + str(i+1) + ".png")
    plt.close()
    ...
    for i, array in enumerate(grads):
        plt.imshow(array)
        plt.savefig(folder_path_sample+"/integrated_grad_value_" + str(i+1) + ".png")
        plt.close()
    ...
    for i, array in enumerate(dl_plots):
        plt.imshow(array)
        plt.savefig(folder_path_sample+"/deep_lift_value_" + str(i+1) + ".png")
        plt.close()
